 ...
 * 길이는 mini `@n` 가중치로 표현.
 * 음색: 단일 WebAudio 신스 (GM/SOUND 톤 순회는 보류).
 */
...
/** 고정 음색 — 지속(clip)이 되는 신스. GM pluck는 고무줄감. */
const TONE = { sound: "sawtooth", cutoff: 1400 } as const;
  ...
  for (let bar = 0; bar < BARS; bar++) {
    ...
    while (step < BAR_STEPS) {
      ...
      if (isAttack(art) && degree !== null) {
        ...
        // 뮤트는 짧게, 나머지는 hold까지 이어서 한 음 (링 = 지속)
 ...
 * 코드는 블록 보이싱으로 길이(@n)만큼 지속 — 스트럼 n 분리는 보류.
 */

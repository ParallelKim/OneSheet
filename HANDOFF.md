...
- **스튜디오 (`/studio`)**: 8슬롯 + CHAIN(채운 순 `cat` 재생). 저장 **`?u=`**. 활성 슬롯에 심플 엔진 임베드.
  - 슬롯·CHAIN·SIMPLE/STUDIO 칩은 **패드 그리드 아래** 도크.
  - 심플↔스튜디오: 차트는 고정, **하단 도크만 Y축 회전문** (`rotateY`). 슬롯 상태 유지.
  - 슬롯: 탭=선택/생성, **길게 누르기=비우기** (힌트 문구 없음).
  - CHAIN은 ON/OFF 글자 없이 점등 토글.
...
- 심플↔스튜디오 → 하단 도크 **Y-flip** 회전문 (차트 고정)
